...
import type { Emits, TextInputHiddenProps } from '.'

defineEmits<Emits>()

const { required, type } = defineProps<TextInputHiddenProps>()
...
<template>
  <div class="text-input-hidden">
    <input :name="name" :required="required" :type="type ?? 'text'" v-model="model" />
    <CheckIcon
      :class="{ inactive: model === original }"
      class="check"
      @click="
        () => {
          original = model
