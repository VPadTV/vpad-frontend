...
import LoadingPage from '@/modules/shared/components/LoadingComponent/LoadingComponent.vue'
import TextAreaInput from '@/modules/shared/components/TextAreaInputField';
import InputField from '@/modules/shared/components/InputField'
import UserHeader from '@/modules/authentication/components/UserHeader.vue'
...
import { loadOrGetUserRef } from '@/modules/user/composables';
...
async function saveClicked() {
    const response = await UserRepository.update(original.value!.id!, {
        ...editedUser.value
    })
    if (response) {
...
watchEffect(() => {
    if (original.value) editedUser.value = { ...original.value }
})
</script>
